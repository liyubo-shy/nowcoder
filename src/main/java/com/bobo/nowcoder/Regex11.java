...
public class Regex11 {
    public static void main(String[] args) throws IOException {
         ...
         *定义规则模板
         **/
//        Pattern p = Pattern.compile("(.*?)用户名:(.*?),扫描二维码登录成功\\[移动端\\]！(.*?)");
//        Pattern p = Pattern.compile("(.*?)用户名:(.*?),登录成功\\[PDA移动端\\]！(.*?)");
//        Pattern p = Pattern.compile("(.*?)用户名:(.*?),登录成功！(.*?)");
//        Pattern p = Pattern.compile("(.*?)用户名:(.*?),登录成功\\[SPS移动端\\]！(.*?)");
        Pattern p = Pattern.compile("(.*?)用户名: (.*?),登录成功\\[移动端\\]！(.*?)");

        /**
         * 读取文件
         **/
        String fileName = "D:\\javaReadFile\\alllogin.txt";
